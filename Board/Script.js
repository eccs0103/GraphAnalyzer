...
try {
	...
	class EdgeEntity extends Entity {
		...
		isMesh(point) {
			...
			return (
				abs((point.y - pointFrom.y) - (point.x - pointFrom.x) * (pointTo.x - pointFrom.x) / (pointTo.y - pointFrom.y)) < EdgeEntity.#width / 2 &&
				hypot(...point["-"](pointFrom)) > VerticeEntity.diameter &&
				hypot(...point["-"](pointTo)) > VerticeEntity.diameter
			);
		}
